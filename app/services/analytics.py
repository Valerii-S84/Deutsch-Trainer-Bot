...
def format_admin_metrics(snapshot: AdminMetricsSnapshot) -> str:
    ...
    return "\n".join(
        [
            ...
            f"Session Completion heute: {_format_rate(daily.session_completion_rate_today)}",
            f"Progress geöffnet heute: {daily.progress_opened_today}",
            f"Fehler wiederholt heute: {daily.mistakes_repeated_today}",
            f"Aktive Abos: {daily.active_subscriptions}",
            f"Payment-Fehler heute: {daily.payment_errors_today}",
            f"API-Fehler heute: {daily.api_errors_today}",
            "",
            f"Paywall CTR heute: {_format_rate(conversion.paywall_ctr_today)}",
            f"Payment Success heute: {_format_rate(conversion.payment_success_rate_today)}",
            f"Free zu Plus heute: {conversion.free_to_plus_today}",
            f"Plus zu Pro heute: {conversion.plus_to_pro_today}",
            f"Abos abgelaufen heute: {conversion.subscription_expired_today}",
            f"Expiration Recovery 30 Tage: {_format_rate(conversion.expiration_recovery_rate_30d)}",
            "",
            f"Retention D1: {_format_rate(retention.day_1)}",
            f"Retention D7: {_format_rate(retention.day_7)}",
            f"Retention D30: {_format_rate(retention.day_30)}",
        ],
    )
