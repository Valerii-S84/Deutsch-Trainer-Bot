...
def _catalog_service(*, themes: list[QuizTheme] | None = None, error: Exception | None = None) -> SimpleNamespace:
...
async def test_theme_selection_handles_empty_and_unavailable_catalog(monkeypatch) -> None:
    ...
    monkeypatch.setattr(theme_handlers, "_catalog_service", lambda: _catalog_service(themes=[]))
    await theme_handlers.open_theme_selection(callback)
    ...
    monkeypatch.setattr(
        theme_handlers,
        "_catalog_service",
        lambda: _catalog_service(error=LocalCatalogNotConfiguredError("missing catalog")),
    )
...
@pytest.mark.asyncio
async def test_level_selection_rolls_back_persist_errors_and_still_uses_local_catalog(monkeypatch) -> None:
    ...
    monkeypatch.setattr(level_handlers, "_catalog_service", lambda: _catalog_service())
...
async def test_level_selection_reports_empty_catalog_without_user_context(monkeypatch) -> None:
    ...
    monkeypatch.setattr(level_handlers, "_catalog_service", lambda: _catalog_service(themes=[]))
    await level_handlers.level_selected(callback)
    assert callback.message.answer.await_args.args[0] == THEME_EMPTY_STATE_TEXT
